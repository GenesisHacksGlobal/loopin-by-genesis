  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4 animate-in fade-in duration-300">
      ...
      <div className="card-feature-light text-center relative overflow-hidden">
        ...
        <h2 className="font-display-lg text-2xl text-white font-light tracking-tight">{event.name}</h2>
        <p className="font-body-md text-xs text-[#64748d] mt-0.5">{event.location}</p>
      </div>

      {/* Main Digital QR Pass Container (card-dashboard-mockup) */}
      <div className="card-dashboard-mockup relative overflow-hidden shadow-2xl space-y-4">
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="flex items-center space-x-3 mb-4 w-full">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-heading-lg text-lg text-white font-light truncate">{user.fullName}</h3>
              <p className="font-body-md text-xs text-[#b9b9f9] font-medium truncate">{user.roleTitle}</p>
          ...
          {/* QR Code Container */}
          <div className="relative p-4 rounded-2xl bg-white shadow-2xl border-4 border-[#533afd]/20 hover:scale-[1.01] transition-transform">
            <QRCodeSVG
              value={payloadString}
              size={200}
              bgColor={"#FFFFFF"}
          ...
          <div className="w-full mt-4 space-y-1.5 font-tabular">
            <div className="flex items-center justify-between text-[11px] text-[#64748d]">
              ...
              <button
                ...
                className="text-[#665efd] hover:text-white transition-colors font-medium"
              >
                Refresh Now
              </button>
            </div>
            <div className="w-full h-1.5 bg-[#1c1e54] rounded-full overflow-hidden">
              <div
          ...
          <div className="w-full mt-4 p-3 rounded-xl bg-[#1c1e54]/60 border border-[#a8c3de]/15 text-center">
            <span className="text-[10px] uppercase font-bold text-[#64748d] tracking-wider block mb-0.5">
              Event Pitch & Request
            </span>
            <p className="font-body-md text-xs text-slate-200 italic">
              "{user.pitch}"
            </p>
          </div>

          {/* Privacy Status Summary */}
          <div className="w-full mt-4 pt-3 border-t border-[#e3e8ee]/10 flex items-center justify-between text-xs text-[#64748d]">
